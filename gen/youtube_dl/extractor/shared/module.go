 ...
 *
 * shared/module.go: transpiled from https://github.com/ytdl-org/youtube-dl/blob/master/youtube_dl/extractor/shared.py
 */
	...
	λ.InitModule(func() {
		...
		SharedBaseIE = λ.Cal(λ.TypeType, λ.NewStr("SharedBaseIE"), λ.NewTuple(InfoExtractor), func() λ.Dict {
			...
			SharedBaseIE__real_extract = λ.NewFunction("_real_extract",
				...
				func(λargs []λ.Object) λ.Object {
					...
					ϒtitle = λ.Cal(λ.GetAttr(ϒself, "_extract_title", nil), ϒwebpage)
					ϒfilesize = λ.Cal(ϒint_or_none, λ.Cal(λ.GetAttr(ϒself, "_extract_filesize", nil), ϒwebpage))
		...
		VivoIE = λ.Cal(λ.TypeType, λ.NewStr("VivoIE"), λ.NewTuple(SharedBaseIE), func() λ.Dict {
			var (
				...
				VivoIE__extract_filesize  λ.Object
				VivoIE__extract_title     λ.Object
			...
			VivoIE__TEST = λ.NewDictWithTable(map[λ.Object]λ.Object{
				...
				λ.NewStr("info_dict"): λ.NewDictWithTable(map[λ.Object]λ.Object{
					...
					λ.NewStr("filesize"): λ.NewInt(515659),
				}),
			})
			VivoIE__extract_title = λ.NewFunction("_extract_title",
				[]λ.Param{
					{Name: "self"},
					{Name: "webpage"},
				},
				...
				func(λargs []λ.Object) λ.Object {
					...
					ϒtitle = λ.Call(λ.GetAttr(ϒself, "_html_search_regex", nil), λ.NewArgs(
						λ.NewStr("data-name\\s*=\\s*([\"\\'])(?P<title>(?:(?!\\1).)+)\\1"),
						ϒwebpage,
						λ.NewStr("title"),
					), λ.KWArgs{
					...
					if λ.IsTrue(ϒtitle) {
						ϒext = λ.Cal(ϒdetermine_ext, ϒtitle)
						if λ.IsTrue(λ.NewBool(λ.Contains(KNOWN_EXTENSIONS, λ.Cal(λ.GetAttr(ϒext, "lower", nil))))) {
							ϒtitle = λ.GetItem(λ.Cal(λ.GetAttr(ϒtitle, "rpartition", nil), λ.Add(λ.NewStr("."), ϒext)), λ.NewInt(0))
						}
						return ϒtitle
					}
					return λ.Cal(λ.GetAttr(ϒself, "_og_search_title", nil), ϒwebpage)
				})
			VivoIE__extract_filesize = λ.NewFunction("_extract_filesize",
				[]λ.Param{
					{Name: "self"},
					{Name: "webpage"},
				},
				...
				func(λargs []λ.Object) λ.Object {
					...
					return λ.Cal(ϒparse_filesize, λ.Call(λ.GetAttr(ϒself, "_search_regex", nil), λ.NewArgs(
						λ.NewStr("data-type=[\"\\']video[\"\\'][^>]*>Watch.*?<strong>\\s*\\((.+?)\\)"),
						ϒwebpage,
						λ.NewStr("filesize"),
					), λ.KWArgs{
			...
			return λ.NewDictWithTable(map[λ.Object]λ.Object{
				...
				λ.NewStr("_extract_filesize"):  VivoIE__extract_filesize,
				λ.NewStr("_extract_title"):     VivoIE__extract_title,
