 ...
 *
 * viddler/module.go: transpiled from https://github.com/rg3/youtube-dl/blob/master/youtube_dl/extractor/viddler.py
 */
...
func init() {
	λ.InitModule(func() {
		...
		ViddlerIE = λ.Cal(λ.TypeType, λ.NewStr("ViddlerIE"), λ.NewTuple(InfoExtractor), func() λ.Dict {
			...
			ViddlerIE__VALID_URL = λ.NewStr("https?://(?:www\\.)?viddler\\.com/(?:v|embed|player)/(?P<id>[a-z0-9]+)")
			...
			ViddlerIE__real_extract = λ.NewFunction("_real_extract",
				...
				func(λargs []λ.Object) λ.Object {
					...
					ϒsecret = λ.GetItem(λ.Cal(λ.GetAttr(ϒqs, "get", nil), λ.NewStr("secret"), λ.NewList(λ.None)), λ.NewInt(0))
					if λ.IsTrue(ϒsecret) {
						λ.SetItem(ϒquery, λ.NewStr("secret"), ϒsecret)
					}
					ϒheaders = λ.NewDictWithTable(map[λ.Object]λ.Object{
						λ.NewStr("Referer"): λ.NewStr("http://static.cdn-ec.viddler.com/js/arpeggio/v2/embed.html"),
					})
					ϒrequest = λ.Cal(ϒsanitized_Request, λ.Mod(λ.NewStr("http://api.viddler.com/api/v2/viddler.videos.getPlaybackDetails.json?%s"), λ.Cal(ϒcompat_urllib_parse_urlencode, ϒquery)), λ.None, ϒheaders)
					ϒdata = λ.GetItem(λ.Cal(λ.GetAttr(ϒself, "_download_json", nil), ϒrequest, ϒvideo_id), λ.NewStr("video"))
					ϒformats = λ.NewList()
