 ...
 *
 * dvtv/module.go: transpiled from https://github.com/rg3/youtube-dl/blob/master/youtube_dl/extractor/dvtv.py
 */
		...
		DVTVIE = λ.Cal(λ.TypeType, λ.NewStr("DVTVIE"), λ.NewTuple(InfoExtractor), func() λ.Dict {
			...
			DVTVIE__TESTS = λ.NewList(
				...
				λ.NewDictWithTable(map[λ.Object]λ.Object{
					...
					λ.NewStr("info_dict"): λ.NewDictWithTable(map[λ.Object]λ.Object{
						λ.NewStr("title"): λ.NewStr("re:^DVTV 16\\. 12\\. 2014: útok Talibanu, boj o kliniku, uprchlíci"),
			...
			DVTVIE__parse_video_metadata = λ.NewFunction("_parse_video_metadata",
				...
				func(λargs []λ.Object) λ.Object {
					...
					if λ.IsTrue(ϒlive_js) {
						λ.Cal(λ.GetAttr(ϒdata, "update", nil), λ.Call(λ.GetAttr(ϒself, "_parse_json", nil), λ.NewArgs(
							ϒlive_js,
							ϒvideo_id,
						), λ.KWArgs{
							{Name: "transform_source", Value: ϒjs_to_json},
					...
					ϒformats = λ.NewList()
					τmp0 = λ.Cal(λ.BuiltinIter, λ.GetItem(ϒdata, λ.NewStr("sources")))
					for {
						if τmp1 = λ.NextDefault(τmp0, λ.AfterLast); τmp1 == λ.AfterLast {
							break
						}
						ϒvideo = τmp1
						ϒvideo_url = λ.Cal(λ.GetAttr(ϒvideo, "get", nil), λ.NewStr("file"))
					...
					return λ.NewDictWithTable(map[λ.Object]λ.Object{
						...
						λ.NewStr("timestamp"):   λ.Cal(ϒint_or_none, λ.Cal(λ.GetAttr(ϒdata, "get", nil), λ.NewStr("pubtime"))),
			...
			DVTVIE__real_extract = λ.NewFunction("_real_extract",
				...
				func(λargs []λ.Object) λ.Object {
					...
					ϒlive_item = λ.Call(λ.GetAttr(ϒself, "_search_regex", nil), λ.NewArgs(
						λ.NewStr("(?s)embedData[0-9a-f]{32}\\.asset\\.liveStarter\\s*=\\s*(\\{.+?\\});"),
						ϒwebpage,
						λ.NewStr("video"),
					), λ.KWArgs{
						{Name: "default", Value: λ.None},
					...
					ϒitem = λ.Call(λ.GetAttr(ϒself, "_search_regex", nil), λ.NewArgs(
						λ.NewStr("(?s)embedData[0-9a-f]{32}\\[[\"\\']asset[\"\\']\\]\\s*=\\s*(\\{.+?\\});"),
						ϒwebpage,
						λ.NewStr("video"),
					), λ.KWArgs{
						{Name: "default", Value: λ.None},
					...
					if λ.IsTrue(ϒitem) {
						return λ.Cal(λ.GetAttr(ϒself, "_parse_video_metadata", nil), ϒitem, ϒvideo_id, ϒlive_item)
					}
					ϒitems = λ.Cal(Ωre.ϒfindall, λ.NewStr("(?s)BBX\\.context\\.assets\\['[0-9a-f]{32}'\\]\\.push\\(({.+?})\\);"), ϒwebpage)
					if λ.IsTrue(λ.NewBool(!λ.IsTrue(ϒitems))) {
						ϒitems = λ.Cal(Ωre.ϒfindall, λ.NewStr("(?s)var\\s+asset\\s*=\\s*({.+?});\\n"), ϒwebpage)
					}
					if λ.IsTrue(ϒitems) {
						return λ.NewDictWithTable(map[λ.Object]λ.Object{
							...
							λ.NewStr("entries"): λ.Cal(λ.ListType, λ.Cal(λ.NewFunction("<generator>",
								nil,
								0, false, false,
								func(λargs []λ.Object) λ.Object {
									return λ.NewGenerator(func(λgen λ.Generator) λ.Object {
										...
										for {
											...
											ϒi = τmp1
											λgen.Yield(λ.Cal(λ.GetAttr(ϒself, "_parse_video_metadata", nil), ϒi, ϒvideo_id))
										}
										return λ.None
									})
								}))),
