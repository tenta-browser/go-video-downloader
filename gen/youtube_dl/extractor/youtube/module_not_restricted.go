...
func init() {
	λ.InitModule(func() {
		...
		ϒparse_qs = λ.NewFunction("parse_qs",
			[]λ.Param{
				{Name: "url"},
			},
			0, false, false,
			func(λargs []λ.Object) λ.Object {
				...
				return λ.Cal(Ωparse.ϒparse_qs, λ.GetAttr(λ.Cal(Ωparse.ϒurlparse, ϒurl), "query", nil))
			})
		...
		YoutubeIE = λ.Cal(λ.TypeType, λ.StrLiteral("YoutubeIE"), λ.NewTuple(YoutubeBaseInfoExtractor), func() λ.Dict {
			...
			YoutubeIE_suitable = λ.NewFunction("suitable",
				[]λ.Param{
					{Name: "cls"},
					{Name: "url"},
				},
				...
				func(λargs []λ.Object) λ.Object {
					...
					ϒqs = λ.Cal(ϒparse_qs, ϒurl)
					if λ.IsTrue(λ.GetItem(λ.Calm(ϒqs, "get", λ.StrLiteral("list"), λ.NewList(λ.None)), λ.IntLiteral(0))) {
						return λ.False
					}
					return λ.Calm(λ.Cal(λ.SuperType, YoutubeIE, ϒcls), "suitable", ϒurl)
				})
			YoutubeIE_suitable = λ.Cal(λ.ClassMethodType, YoutubeIE_suitable)
				...
				func(λargs []λ.Object) λ.Object {
					...
					ϒget_text = λ.NewFunction("get_text",
						...
						func(λargs []λ.Object) λ.Object {
							...
							ϒtext = λ.Calm(ϒx, "get", λ.StrLiteral("simpleText"))
							if λ.IsTrue(func() λ.Object {
								if λv := ϒtext; !λ.IsTrue(λv) {
									return λv
								} else {
									return λ.Cal(λ.BuiltinIsInstance, ϒtext, ϒcompat_str)
								}
							}()) {
								return ϒtext
							}
							ϒruns = λ.Calm(ϒx, "get", λ.StrLiteral("runs"))
							if !λ.IsTrue(λ.Cal(λ.BuiltinIsInstance, ϒruns, λ.ListType)) {
								return λ.None
							}
							return λ.Calm(λ.StrLiteral(""), "join", λ.Cal(λ.ListType, λ.Cal(λ.NewFunction("<generator>",
								...
								func(λargs []λ.Object) λ.Object {
									return λ.NewGenerator(func(λgy λ.Yielder) λ.Object {
										...
										τmp0 = λ.Cal(λ.BuiltinIter, ϒruns)
										for {
											if τmp1 = λ.NextDefault(τmp0, λ.AfterLast); τmp1 == λ.AfterLast {
												break
											}
											ϒr = τmp1
											if λ.IsTrue(λ.Cal(λ.BuiltinIsInstance, λ.Calm(ϒr, "get", λ.StrLiteral("text")), ϒcompat_str)) {
					...
					for {
						...
						if λ.IsTrue(λ.Calm(ϒkeyword, "startswith", λ.StrLiteral("yt:stretch="))) {
							ϒmobj = λ.Cal(Ωre.ϒsearch, λ.StrLiteral("(\\d+)\\s*:\\s*(\\d+)"), ϒkeyword)
							if λ.IsTrue(ϒmobj) {
								τmp2 = λ.UnpackIterable(λ.Cal(λ.NewFunction("<generator>",
									nil,
									0, false, false,
									func(λargs []λ.Object) λ.Object {
										return λ.NewGenerator(func(λgy λ.Yielder) λ.Object {
											...
											for {
												...
												ϒv = τmp1
												λgy.Yield(λ.Cal(λ.FloatType, ϒv))
											}
											return λ.None
										})
									})), 2)
						...
						for {
							...
							if λ.IsTrue(ϒvpir) {
								...
								if λ.IsTrue(ϒsbr_tooltip) {
									τmp2 = λ.UnpackIterable(λ.Calm(ϒsbr_tooltip, "split", λ.StrLiteral(" / ")), 2)
		...
		YoutubeTabIE = λ.Cal(λ.TypeType, λ.StrLiteral("YoutubeTabIE"), λ.NewTuple(YoutubeBaseInfoExtractor), func() λ.Dict {
			...
			YoutubeTabIE__real_extract = λ.NewFunction("_real_extract",
				...
				func(λargs []λ.Object) λ.Object {
					...
					if λ.IsTrue(ϒtabs) {
						return λ.Calm(ϒself, "_extract_from_tabs", ϒitem_id, ϒwebpage, ϒdata, ϒtabs)
					}
		...
		YoutubePlaylistIE = λ.Cal(λ.TypeType, λ.StrLiteral("YoutubePlaylistIE"), λ.NewTuple(InfoExtractor), func() λ.Dict {
			...
			YoutubePlaylistIE_suitable = λ.NewFunction("suitable",
				...
				func(λargs []λ.Object) λ.Object {
					...
					ϒqs = λ.Cal(ϒparse_qs, ϒurl)
					if λ.IsTrue(λ.GetItem(λ.Calm(ϒqs, "get", λ.StrLiteral("v"), λ.NewList(λ.None)), λ.IntLiteral(0))) {
						return λ.False
					}
