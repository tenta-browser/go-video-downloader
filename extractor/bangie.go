...
func (self *BangIE) _real_extract(url string) map[string]interface{} {
	...
	api_key := (self).SearchRegex(`apiKey\s*:\s*["\']([^"\']+)["\']`, webpage, `api_key`, nil, true, 0, nil)
	if !((api_key).IsSet() && (api_key.Get()) != "") {
		panic(rnt.PyExtractorError(`LOGIN_REQUIRED`, true, rnt.OptString{}))
	}
	mainjs_url := (self).SearchRegex(`<script[^>]+?src=["\'](?P<mainjs_url>/assets/js/main.[a-z0-9]+.js)`, webpage, `main.js url`, rnt.NoDefault, true, 0, nil)
	mainjs_url = rnt.AsOptString(rnt.URLJoin(url, mainjs_url))
	mainjs := (self).DownloadWebpage(mainjs_url.Get(), video_id, rnt.AsOptString(`Downloading main.js`), rnt.OptString{}, true, 1, 5, rnt.OptString{}, rnt.OptString{}, map[string]interface{}{}, map[string]interface{}{})
	elasticsearch_url := (self).SearchRegex(`"elasticsearch.url"\s*:\s*"([^"]+)"`, mainjs, `Elasticsearch url`, rnt.NoDefault, true, 0, nil)
	elasticsearch_auth := (self).SearchRegex(`"elasticsearch.auth"\s*:\s*"([^"]+)"`, mainjs, `Elasticsearch auth`, rnt.NoDefault, true, 0, nil)
