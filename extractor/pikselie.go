...
func (self *PikselIE) _real_extract(url string) rnt.SDict {
	...
	if rnt.IsTruthy(m3u8_url) {
		formats = append(formats, τ_conv_Ld_to_Lα((self).ExtractM3U8Formats(rnt.CastToString(m3u8_url), video_id, rnt.AsOptString("mp4"), rnt.AsOptString("m3u8_native"), rnt.OptFloat{}, rnt.AsOptString("hls"), rnt.OptString{}, rnt.OptString{}, false, false))...)
	}
