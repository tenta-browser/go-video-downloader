 ...
 *
 * mlbie.go: Automatically transpiled from https://github.com/rg3/youtube-dl/blob/master/youtube_dl/extractor/mlb.py
 */
...
func NewMLBIE() rnt.InfoExtractor {
	var (
		IE_NAME         string
		_CONTENT_DOMAIN string
		_VALID_URL      string
...
func (self *MLBIE) Key() string {
	return "MLB"
}

func (self *MLBIE) Name() string {
	return self.IE_NAME
}

func (self *MLBIE) _real_extract(url string) rnt.SDict {
	var (
		cuts           interface{}
		ext            string
		formats        []interface{}
...
func (self *MLBIE) Extract(url string) (rnt.ExtractorResult, error) {
	return rnt.RunExtractor(url, self.Context, self._real_extract)
}

func init() {
	registerFactory("MLB", NewMLBIE)
}
