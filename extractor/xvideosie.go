...
func (self *XVideosIE) _real_extract(url string) rnt.SDict {
	...
	for _, τel := range rnt.ReFindAllMulti("setVideo([^(]+)\\(([\"\\'])(http.+?)\\2\\)", webpage, 0) {
		...
		if (format_id) == ("hls") {
			formats = append(formats, τ_conv_Ld_to_Lα((self).ExtractM3U8Formats(format_url, video_id, rnt.AsOptString("mp4"), rnt.AsOptString("m3u8_native"), nil, rnt.AsOptString("hls"), rnt.OptString{}, rnt.OptString{}, false, false))...)
		} else {
