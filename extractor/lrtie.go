...
func (self *LRTIE) _real_extract(url string) rnt.SDict {
	...
	for _, τel := range rnt.ReFindAllMulti("file\\s*:\\s*([\"\\'])(?P<url>(?:(?!\\1).)+)\\1", webpage, 0) {
		...
		if (ext) == ("m3u8") {
			formats = append(formats, τ_conv_Ld_to_Lα((self).ExtractM3U8Formats(file_url, video_id, rnt.AsOptString("mp4"), rnt.AsOptString("m3u8_native"), nil, rnt.OptString{}, rnt.OptString{}, rnt.OptString{}, false, false))...)
		} else {
			if (ext) == ("mp3") {
